...
#define TEST_EVAL_ASSIGN_WITH_MATH(data, oper, expected) \
	...
	Variable *var_a = make_variable(&TYPE_INT, U"a", false); \
	SkullInt num = (data); \
	variable_write(var_a, &num); \
	scope_add_var(scope, var_a); \
	const char32_t *error = NULL; \
	AstNode *node = make_ast_tree(U"a " oper U" a", &error); \
	Variable *var_b = make_variable(&TYPE_INT, U"b", false); \
...
#define TEST_EVAL_ASSIGN_DIFFERENT_TYPES(type_a, real_type_a, value_a, oper, type_b, real_type_b, value_b, expected) \
	Scope *scope = make_scope(); \
	Variable *var_a = make_variable(type_a, U"a", false); \
	real_type_a data_a = (value_a); \
	variable_write(var_a, &data_a); \
	scope_add_var(scope, var_a); \
	Variable *var_b = make_variable(type_b, U"b", false); \
	...
	Variable *var_c = make_variable(&TYPE_INT, U"c", false); \
...
#define TEST_EVAL_ASSIGN_OPER_AVAILABLE(oper, expected) \
	Scope *scope = make_scope(); \
	Variable *var_a = make_variable(&TYPE_BOOL, U"a", false); \
	scope_add_var(scope, var_a); \
	const char32_t *error = NULL; \
	AstNode *node = make_ast_tree(U"a " oper U" a", &error); \
	Variable *var_b = make_variable(&TYPE_INT, U"b", false); \
